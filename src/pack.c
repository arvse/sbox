 ...
 */
static int sbox_pack_archive_stream ( uint32_t options, struct ar_ostream *ostream,
    const char *files[], size_t nfiles )
{
    ...
    if ( options & OPTION_LZ4 )
    {
        header.comp = COMP_LZ4;
    ...
    if ( ostream->put_header ( ostream, &header ) < 0 )
    {
    ...
    const char *files[], size_t nfiles )
{
    ...
    /* Assign IO stream */
    if ( !( io = io_ostream_from ( fd, password ) ) )
    {
        close ( fd );
        return -1;
    }
    ...
    {
#ifdef ENABLE_LZ4
        ostream = lz4_ostream_open ( io, level );
#else
        UNUSED ( level );
    ...
    status = sbox_pack_archive_stream ( options, ostream, files, nfiles );
