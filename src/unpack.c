 ...
 */
static int sbox_unpack_parse ( const struct header_t *header, const struct entity_t *entity_table,
 ...
 */
static int sbox_extract_file ( struct unpack_context_t *context, struct entity_t *entity )
{
    int fd;
    ssize_t len;
    uint32_t left;
    ...
    if ( context->options & OPTION_LISTONLY )
    {
 ...
 */
static int sbox_extract_next ( struct unpack_context_t *context, struct node_t *node )
{
    size_t path_len = 0;
    ...
    if ( ~context->options & OPTION_NOPATHS || ~node->entity.mode & S_IFDIR )
    {
        ...
        if ( sbox_extract_file ( context, &node->entity ) < 0 )
        {
            return -1;
        }
    }

    if ( sbox_extract_next ( context, node->sub ) < 0 )
    {
        return -1;
    }
    ...
    return sbox_extract_next ( context, node->next );
}

/** 
 * Load metadata of archive files
 */
static int sbox_unpack_archive_load ( struct header_t *header, struct ar_istream *istream,
    uint32_t options )
{
    int status = 0;
    int verstatus;
    ...
    /* Parse archive nodes into memory */
    if ( sbox_unpack_parse ( ( const struct header_t * ) header,
    ...
    /* Extract files */
    status = sbox_extract_next ( &context, root );
...
int sbox_unpack_archive ( const char *archive, uint32_t options, const char *password )
{
    ...
    /* Assign IO stream */
    if ( !( io = io_istream_from ( fd, password ) ) )
    {
        close ( fd );
        return -1;
    }
    ...
    /* Open archive stream */
    if ( !( istream = plain_istream_open ( io ) ) )
    {
        io->close ( io );
        return -1;
    }
    ...
    if ( header.comp == COMP_LZ4 )
    {
        istream->close ( istream );
#ifdef ENABLE_LZ4
        if ( !( istream = lz4_istream_open ( io ) ) )
        {
            io->close ( io );
            return -1;
        }
#else
        fprintf ( stderr, "lz4 not enabled.\n" );
        errno = EINVAL;
        io->close ( io );
        return -1;
#endif
    ...
    status = sbox_unpack_archive_load ( &header, istream, options );
